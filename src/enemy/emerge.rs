...
trait EnemyEmerger {
	fn emerge(&self, frame: i128, assert_server: &EnemyAssetsHolder, commands: &mut Commands);
}
...
impl EnemyEmerger for EnemyEmerge {
	fn emerge(&self, frame: i128, assets_holder: &EnemyAssetsHolder, commands: &mut Commands) {
			...
			for emerge in emerge_list.iter() {
				commands
					.spawn_bundle(SpriteBundle {
						...
						transform: Transform {
							translation: emerge.initial_position,
							scale: enemy_size,
							..Default::default()
						},
...
fn emerge_enemy_system(
	...
	for ev in ev_emerge_frame_changed.iter() {
		emerger.emerge(ev.0, &assets_holder, &mut commands);
	}
}
